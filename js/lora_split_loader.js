// LoRA Split Loader (CCN) -- two fixed rows blended by a balance slider.
//
// Rows serialize as widget values named "lora_a" / "lora_b":
//   { on: bool, lora: string, slot: "a" | "b" }
// The Python side (lora_split_loader.py) derives each row's strength from
// the balance and strength_multiplier widgets; the number drawn on a row is
// a read-only preview of that math. Trigger chips are display-only and
// never serialized; they come from POST /ccn/lora_triggers on pick, on
// workflow configure, and via the Refresh Triggers button (which bypasses
// the cache).

import { app } from "../../scripts/app.js";
import { api } from "../../scripts/api.js";

const NODE_NAME = "CCN_LoraSplitLoader";
const ROW_TYPE = "CCN_LORA_SPLIT_ROW";
const ROW_SLOTS = ["a", "b"];
...
async function fetchLoraList() {
  try {
    ...
    console.warn("[CCN LoraSplitLoader] failed to fetch lora list", err);
  }
...
// Shared with the multi loader so both choosers sort alike.
try {
...
async function fetchLoraEntries() {
  try {
    ...
    console.warn("[CCN LoraSplitLoader] /ccn/lora_filter/list unavailable, using name-only list", err);
  }
...
async function fetchTriggers(names, { force = false } = {}) {
  ...
  try {
    ...
    console.warn("[CCN LoraSplitLoader] trigger fetch failed", err);
  }
}
...
function scalarValue(node, name, fallback) {
  const w = node.widgets?.find((x) => x.name === name);
  return typeof w?.value === "number" && Number.isFinite(w.value) ? w.value : fallback;
}

// The strength Python will derive for a slot. If balance is converted to
// an input this shows the residual widget value, not the link-fed one --
// cosmetic only, Python is authoritative.
function effectiveStrength(node, slot) {
  const b = Math.min(1, Math.max(0, scalarValue(node, "balance", 0.5)));
  const m = scalarValue(node, "strength_multiplier", 1.0);
  return (slot === "b" ? b : 1 - b) * m;
}
...
function makeRow(node, slot) {
  const widget = {
    type: ROW_TYPE,
    name: `lora_${slot}`,
    value: { on: true, lora: "None", slot },
    _hit: {},
    _copied: null,
    ...
    serializeValue() {
      const { on, lora, slot: rowSlot } = this.value;
      return { on, lora, slot: rowSlot };
    },
    ...
    draw(ctx, drawNode, width, y, height) {
      ...
      // Slot badge, display only.
      const badgeX0 = margin + 24;
      const badgeX1 = badgeX0 + 16;
      ctx.fillStyle = "#3a3a4a";
      ctx.beginPath();
      ctx.roundRect(badgeX0, y + 4, badgeX1 - badgeX0, lineH - 8, 3);
      ctx.fill();
      ctx.fillStyle = on ? "#ddd" : "#888";
      ctx.font = "9px monospace";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(this.value.slot === "b" ? "B" : "A", (badgeX0 + badgeX1) / 2, midY);

      // Read-only effective strength, right-aligned. Not clickable: the
      // balance and strength_multiplier widgets own this value.
      ctx.font = "12px Arial";
      const fieldW = 52;
      const strengthX1 = width - margin - 6;
      const strengthX0 = strengthX1 - fieldW;
      ctx.textAlign = "left";
      ctx.fillStyle = LiteGraph.WIDGET_SECONDARY_TEXT_COLOR;
      ctx.fillText(
        effectiveStrength(drawNode, this.value.slot).toFixed(2),
        strengthX0 + 8, midY);

      // Name fills the remaining middle.
      const nameX0 = badgeX1 + 8;
      const nameX1 = strengthX0 - 8;
      let display = this.value.lora === "None"
        ? "click to choose…"
        : baseName(this.value.lora);
      ctx.fillStyle = this.value.lora === "None" || !on
...
// Idempotent: creates any missing fixed row, keeping rows before the
// Refresh Triggers button. Row order is always [a, b].
function ensureFixedRows(node) {
  node.widgets = node.widgets ?? [];
  for (const slot of ROW_SLOTS) {
    if (rowWidgets(node).some((r) => r.value?.slot === slot)) continue;
    const row = makeRow(node, slot);
    const buttonIndex = node.widgets.findIndex((w) => w === node._ccnRefreshButton);
    if (buttonIndex === -1) {
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.configure = function (info) {
      // Both rows must exist BEFORE LiteGraph applies widgets_values by
      // index, so the arrays line up.
      ensureFixedRows(this);
      const result = origConfigure?.apply(this, arguments);
      if (info?.widgets_values) {
        // Re-sync row values defensively (index alignment can differ
        // across frontend versions when non-serialized widgets are
        // present). Saved values match rows by slot, index as fallback.
        const rowValues = info.widgets_values.filter(isRowValue);
        const rows = rowWidgets(this);
        rows.forEach((row, i) => {
          const slot = ROW_SLOTS[i] ?? row.value?.slot;
          const saved = rowValues.find((v) => v.slot === slot) ?? rowValues[i];
          row.value = { on: true, lora: "None", ...(saved ?? {}), slot };
          row.name = `lora_${slot}`;
        });

        // Positional application can stuff a row dict into the scalar
        // widgets. Recover: the lone boolean is `enabled`; the finite
        // numbers map to [balance, strength_multiplier] in widget
        // declaration order.
        ...
        const nums = info.widgets_values.filter(
          (v) => typeof v === "number" && Number.isFinite(v));
        const balanceWidget = this.widgets.find((w) => w.name === "balance");
        if (balanceWidget) {
          if (nums.length >= 1) balanceWidget.value = Math.min(1, Math.max(0, nums[0]));
          else if (!Number.isFinite(balanceWidget.value)) balanceWidget.value = 0.5;
        }
        const multWidget = this.widgets.find(
          (w) => w.name === "strength_multiplier");
        if (multWidget) {
          if (nums.length >= 2) multWidget.value = nums[1];
